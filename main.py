...
def run():
    ...
    if action == 'train':
        ...
        if model_name == "SAHC":
            ...
            video_traindataset = dataset.TestVideoDataset(opt.dataset, opt.dataset_path.format(opt.dataset) + '/train_dataset', 5, 'video_feature_tcn')
            video_testdataset = dataset.TestVideoDataset(opt.dataset, opt.dataset_path.format(opt.dataset) + '/test_dataset', 5, 'video_feature_tcn')

            # video_traindataset = dataset.TestVideoDataset(opt.dataset, opt.dataset_path.format(opt.dataset) + '/train_dataset', 5, 'video_feature_resnet50')
            # video_testdataset = dataset.TestVideoDataset(opt.dataset, opt.dataset_path.format(opt.dataset) + '/test_dataset', 5, 'video_feature_resnet50')
    ...
    if args.action == 'extract_video':
        ...
        if model_name == "tcn_video":
            ...
            import model.predictor.tcn as tcn_model
            model = tcn_model.MultiStageModel1(opt)
            import script.tcn as tcn_action

            tcn_action.extract_video(opt, model, video_train_dataloader, video_test_dataloader, device, save_dir="../../Dataset/SAHC/even/")
